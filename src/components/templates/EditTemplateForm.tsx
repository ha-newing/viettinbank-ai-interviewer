...
import { CheckCircle, Loader2, FileText, Settings, Target, RotateCcw, Save } from 'lucide-react'
...
export default function EditTemplateForm({ user, template }: EditTemplateFormProps) {
  ...
  const [form, setForm] = useState({
    ...
    interviewDuration: template.interviewDuration,
    impressionWeight: template.impressionWeight,
    taskPerformanceWeight: template.taskPerformanceWeight,
    logicalThinkingWeight: template.logicalThinkingWeight,
    researchAbilityWeight: template.researchAbilityWeight,
  ...
  // Navigate back to view page on successful update
  useEffect(() => {
    if (state?.success) {
  ...
  return (
    <div className="space-y-6">
      ...
      {hasChanges && !state?.success && (
        <Alert className="border-blue-200 bg-blue-50">
          <AlertDescription className="text-blue-800">
            ℹ️ Bạn có thay đổi chưa lưu. Nhớ lưu trước khi rời khỏi trang.
          </AlertDescription>
        </Alert>
      )}
      ...
      <form action={formAction} className="space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Cập nhật tên và mô tả cho template phỏng vấn
            </CardDescription>
          </CardHeader>
